...
from app.services.catalog import get_or_create_case, get_or_create_suite
...
async def ingest_run(
    ...
    for item in body.results:
        suite = await get_or_create_suite(db, project.id, item.file_path, suite_cache)
        case = await get_or_create_case(db, suite.id, item.test_title, case_cache)
        result = TestResult(
            run_id=run.id,
            case_id=case.id,
            status=item.status,
